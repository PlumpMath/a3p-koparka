...
void main()
    ...
    if(fogFactor>0.996)//fog only version
        ...
        //vec4 walk=vec4(1.0,1.0,1.0,1.0)- step(textureLod(walkmap,texUV), vec4(0.5,0.5,0.5,0.5));
        vec4 walk=texture(walkmap,texUV);
        final=mix(final,  min(vec4(0.0, 0.02, 0.04, 1.0), vec4(fog.rgb, 1.0)), waterFog);
        float final_fog=clamp(fogFactor-walk.r, 0.0, 1.0);
        gl_FragData[0] = mix(final, vec4(fog.rgb, 1.0),final_fog*final_fog)+walk;                
