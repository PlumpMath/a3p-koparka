...
void main()
    ...
    for (int i=0; i<num_lights; ++i)
        ...
        //specular
        R=reflect(L,N)*att;
        specular +=pow(max(dot(R, V), 0.0), 10.0+gloss*100.0)*light_color[i].a*gloss*5.0;
        }
    ...
    vec4 sky_tex= texture(spheremap,vN);
    float stars=sky_tex.g*(1.0-skyColor.a);
    vec3 clouds=sky_tex.r*cloudColor.rgb;
    vec3 env=(skyColor.rgb*(1.0-sky_tex.r))+clouds+stars;        
    color+=(env*max(dot(N,up),-0.2)*0.7+env*0.3)*(gloss+specular)*2.0;
